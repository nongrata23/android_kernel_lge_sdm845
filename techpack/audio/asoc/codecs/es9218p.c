...
struct registry_map es9218p_regmap[] = {
	...
	{ "07_FIILTER_BANDWIDTH_&_SYSTEM_MUTE", ES9218P_FILTER_BAND_SYSTEM_MUTE, 1 },
	{ "08_GPIO1-2_CONFIGURATION", ES9218P_GPIO1_2_CONFIG, 1 },
	{ "09_RESERVED_09", ES9218P_REG09_RESERVED, 1 },
	{ "10_MASTER_MODE_&_SYNC_CONFIGURATION", ES9218P_MASTERMODE_SYNC_CONFIG, 1 },
	...
	{ "49_AUTOMATIC_CLOCK_GEARING_THRESHOLDS_3", ES9218P_CLK_GEAR_THRESH_3, 1 },
	{ "50_AUTOMATIC_CLOCK_GEARING_THRESHOLDS_2", ES9218P_CLK_GEAR_THRESH_2, 1 },
	{ "51_AUTOMATIC_CLOCK_GEARING_THRESHOLDS_1", ES9218P_CLK_GEAR_THRESH_1, 1 },
	...
	{ "72_INPUT_SELECTION_AND_AUTOMUTE_STATUS", ES9218P_INPUT_SEL_AUTOMUTE_STATUS, 0 },
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	for (i = 0, n = 0; i < reg_count; i++) {
		...
		n += scnprintf(buf + n, PAGE_SIZE - n,
			       "%-40s <#%02d>= 0x%02X\n", es9218p_regmap[i].name,
			       es9218p_regmap[i].reg, read_buf);
	}
...
static int es9218_set_avc_volume(struct i2c_client *client, int vol)
{
	...
	ret |= es9218_write_reg(g_es9218_priv->i2c_client, ES9218P_ANALOG_VOL_CTRL,
				value);
...
es9218_sabre_cfg_custom_filter(struct sabre_custom_filter *sabre_filter)
{
	...
	reg = es9218_read_reg(g_es9218_priv->i2c_client, ES9218P_FILTER_BAND_SYSTEM_MUTE);
	...
	rc = es9218_write_reg(g_es9218_priv->i2c_client, ES9218P_FIR_CONFIG, rv);
	if (rc < 0) {
		pr_err("%s: rc = %d return ", __func__, rc);
		return rc;
	}

	rv = es9218_read_reg(g_es9218_priv->i2c_client, ES9218P_FIR_RAM_ADDR - 1);
	...
	es9218_write_reg(g_es9218_priv->i2c_client, ES9218P_FIR_RAM_ADDR - 1, rv);

	/* Select the custom filter roll-off shape. */
	rv = (sabre_filter->shape << 5);
	rv |= 0x80;

	rc = es9218_write_reg(g_es9218_priv->i2c_client, ES9218P_FILTER_BAND_SYSTEM_MUTE, rv);
	if (rc < 0) {
		pr_err("%s: rc = %d return ", __func__, rc);
...
static int es9218p_sabre_lpb2hifi(int mode)
{
	...
	es9218_write_reg(g_es9218_priv->i2c_client, ES9218P_ANALOG_VOL_CTRL, 0x18);
#if ES9218P_DEBUG == 1
	pr_info("%s(): R03 = %X \n", __func__, 0x18);
	mdelay(g_debug_delay);
#endif
	...
	es9218_write_reg(g_es9218_priv->i2c_client, ES9218P_ANALOG_CTRL_OVERRIDE,
	 ...
	 * conflicts.
	 */
	es9218_write_reg(g_es9218_priv->i2c_client, ES9218P_ANALOG_VOL_CTRL, value);
#if ES9218P_DEBUG == 1
	pr_info("%s(): R03 = %X \n", __func__, value);
	mdelay(g_debug_delay);
#endif
...
static int es9218p_sabre_hifi2lofi2lpb(void)
{
	...
	uint8_t rv = es9218_read_reg(g_es9218_priv->i2c_client, ES9218P_FILTER_BAND_SYSTEM_MUTE);
#endif
...
#if WORKAROUND_FOR_LOFI_POP == 1
	if (rv & 1)
		es9218_write_reg(g_es9218_priv->i2c_client, ES9218P_FILTER_BAND_SYSTEM_MUTE,
				 rv |= 0x01);
#endif
...
static int es9218_set_bit_width(unsigned int bit_width, unsigned int ess_mode)
{
	...
	if (ess_mode == ESS_MODE_PCM) {
		...
		case 16:
			...
			ret = es9218_write_reg(g_es9218_priv->i2c_client,
					       ES9218P_INPUT_SELECT, in_cfg_reg);
			break;
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	reg = es9218_read_reg(g_es9218_priv->i2c_client, ES9218P_FILTER_BAND_SYSTEM_MUTE);
	reg |= 0x01;
	es9218_write_reg(g_es9218_priv->i2c_client, ES9218P_FILTER_BAND_SYSTEM_MUTE, reg);
	mdelay(2);
	es9218_sabre_cfg_custom_filter(&sabre_custom_ft[g_sabre_cf_num]);
	reg = es9218_read_reg(g_es9218_priv->i2c_client, ES9218P_FILTER_BAND_SYSTEM_MUTE);
	reg &= ~0x01;
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	err_check =
		es9218_read_reg(g_es9218_priv->i2c_client, ES9218P_MASTERMODE_SYNC_CONFIG);
	if (err_check >= 0) {
		reg_val = err_check;
	} else {
		return -1;
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	es9218_write_reg(g_es9218_priv->i2c_client, ES9218P_MASTERMODE_SYNC_CONFIG,
			 reg_val);
	return 0;
}
				     ...
				     struct es9218_data *pdata)
{
	...
	ret = of_property_read_uint32_t_array(dev->of_node, "dac,va-supply-voltage",
					 vol_suply, 2);
	if (ret < 0) {
...
#if ENABLE_DOP_AUTO_MUTE == 1
		es9218_write_reg(g_es9218_priv->i2c_client, ES9218P_AUTOMUTE_TIME,
				 0x00);
		es9218_write_reg(g_es9218_priv->i2c_client, ES9218P_AUTOMUTE_CONFIG,
				 0x34);
#endif
		/* DoP Playback */
